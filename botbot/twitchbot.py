...
def is_mod(line):
	if "@badges=" in line and "display-name=" in line and 'subscriber=' in line and 'mod=1' in line:
		return True
	else:
		return False
...
def send_message(message,channel,delay=5):
	global delay_dict
...
while True:
	for line in str(s.recv(1024)).split('\\r\\n'):
		...
		if message.startswith("!last "):
			name = message[6:]
			name = name.lower()
			msg = logreader.read_backwards(channel,name)
			send_message(msg,channel)
		...
		if "FUCK" in message.upper():
			msg = "stop breaking rule 2 "+ username +" MiniKeepo"
			send_message(msg,channel)
